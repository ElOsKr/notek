...
function estructuraHTML(id,nombre,color="text-light",status="null"){
    return`
            <div class="d-flex" data-status="${status}">
                <p class="me-2 mt-3 ${id} ${color}">${nombre}</p>
                <div class="${id} me-auto mt-2">
                    <button class="btn btnEnProceso text-primary" data-id="${id}">
                    ...
                    <button class="btn btnPausa text-warning" data-id="${id}">
                    ...
                    <button class="btn btnAcabado text-success" data-id="${id}">
                ...
                <div class="oculto input-group mt-2 ${id} me-auto w-75">
                    <input type="text" class="form-control bg-transparent text-light" value="${nombre}">
                    <button class="btn btnGuardarEditar text-light" data-id="${id}">
        ...
        if(comprobar[0]==1){
            if(comprobar[1]!=""){
                ...
                comprobar[1].forEach(item=>{
                    ...
                    html += estructuraHTML(item.id,item.title,color,item.status);
                })
        ...
        if(this.readyState==4 && this.status==200){
            ...
            if(comprobar==0){
              alert("Error");  
                ...
                var html=estructuraHTML(comprobar,nombreItem);
                document.getElementById("tareas").innerHTML+=html;
                cargarItems();
                $("#listaItemNombre").val("");
        ...
        xhttp.onload = function() {
            if(this.readyState==4 && this.status==200){
                ...
                for (let i = 0; i < comprobar.length; i++) {
                    var color="";
                    if(comprobar[i]['status']=="enProceso"){
                        color="text-primary"
                    }else if(comprobar[i]['status']=="enPausa"){
                        color="text-warning"
                    }else if(comprobar[i]['status']=="completado"){
                        color="text-success"
                    }
                    html +=estructuraHTML(comprobar[i]['id'],comprobar[i]['title'],color,comprobar[i]['status']); 
